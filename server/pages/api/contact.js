...
const handler = next_connect__WEBPACK_IMPORTED_MODULE_0___default()().use(_middleware_mongodb__WEBPACK_IMPORTED_MODULE_1__/* .default */ .Z).post(async (req, res) => {
  const body = req.body; // Extract data
  ...
  const docId = await req.mongoDb.collection("form-responses").insertOne({
    name,
    email,
    comments
  });
...
var __webpack_exports__ = (__webpack_exec__(3187));
